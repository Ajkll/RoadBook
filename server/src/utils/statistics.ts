 ...
 */
export const calculateSessionStatistics = async (sessionId: string) => {
  try {
    ...
    logger.error(`Error calculating session statistics: ${error}`);
    throw error;
  }
};
 ...
 */
export const calculateRoadbookStatistics = async (roadbookId: string) => {
  try {
    ...
    const weatherDistribution = sessions.reduce((dist: Record<string, number>, session) => {
      const weather = session.weather || 'UNKNOWN';
      dist[weather] = (dist[weather] || 0) + 1;
      return dist;
    }, {});

    const daylightDistribution = sessions.reduce((dist: Record<string, number>, session) => {
    ...
    const roadTypesCount: Record<string, number> = {};
    ...
    for (let i = 0; i < sessions.length; i++) {
      ...
      if (lastSessionDate) {
        const daysDiff = Math.floor(
          (sessionDate.getTime() - lastSessionDate.getTime()) / (1000 * 60 * 60 * 24)
        );
    ...
    const daysSinceCreation = Math.floor(
      (new Date().getTime() - new Date(roadbook.createdAt).getTime()) / (1000 * 60 * 60 * 24)
    );

    // Analyse des localisations pour cartographie
    const locations = sessions.flatMap(session => {
      const locs = [];
      if (session.startLocation) locs.push(session.startLocation);
      if (session.endLocation) locs.push(session.endLocation);
      return locs;
    });

    // Identifier les heures de conduite (répartition jour/nuit/crépuscule)
    const timeDistribution = sessions.reduce((dist: Record<string, number>, session) => {
    ...
    logger.error(`Error calculating roadbook statistics: ${error}`);
    throw error;
  }
};
 ...
 */
export const calculateApprenticeStatistics = async (apprenticeId: string) => {
  try {
    ...
    logger.error(`Error calculating apprentice statistics: ${error}`);
    throw error;
  }
};
 ...
 */
export const calculatePeriodMetrics = (sessions: any[], periodDescription: string) => {
  try {
    ...
    logger.error(`Error calculating ${periodDescription} metrics: ${error}`);
    throw error;
  }
};